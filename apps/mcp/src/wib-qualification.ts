...
function validateConstraints(constraints: WibConstraintDefinition[]) {
  ...
  return constraints.map((constraint) => {
    if (!constraint.id.trim() || !constraint.requirement.trim() || !constraint.source_authority.trim()) throw new Error("Every constraint needs id, requirement, and source_authority");
