...
describe("Allegro BRD selected-TP MCP surface", () => {
  it("registers the complete review workflow with resources and Viewer deep links", async () => {
    ...
    expect(source).toContain('review_action: z.enum(["APPROVE", "REJECT", "IGNORE", "REVIEW"])');
  });
});
