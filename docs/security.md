...
- Schematic page and thumbnail IPC validates the artifact ID and page number, resolves only paths recorded inside that artifact's cache directory, and returns image bytes plus structured overlays; the renderer never receives an arbitrary filesystem read primitive.
...
- PDF rendering, OCR, and image processing use packaged local resources. They do not send schematic pages, recognized text, component identities, or graph corrections to a network service. OCR and inferred graph evidence still require selected-path confirmation.
