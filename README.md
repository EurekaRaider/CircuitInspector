...
Six structured MCP tools, progress and cancellation, paginated findings, evidence resources, and stdout reserved for JSON-RPC.
...
The Rust core is the sole authority for manufacturing data, geometry, and verdicts. The MCP does not recalculate distances. The renderer does not parse PCB files or run full spatial queries on its main thread.
...
- 6 TypeScript integration and localization tests passing
